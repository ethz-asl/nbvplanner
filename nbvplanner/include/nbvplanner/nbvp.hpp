...
nbvInspection::Node<stateVec> * nbvInspection::Node<stateVec>::minDist(stateVec s)
{
  ...
  for (typename std::vector<nbvInspection::Node<stateVec> *>::iterator it = children_.begin();
    ...
    double tmpDist = sqrt(
    SQ(s[0] - tmp->state_[0]) + SQ(s[1] - tmp->state_[1]) + SQ(s[2] - tmp->state_[2]));
    if (tmpDist < bestDist) {
      ret = tmp;
      bestDist = tmpDist;
    }
      ...
      nh_private_(nh_private)
{
  ...
  double camTop = M_PI * (pitch - nbvInspection::nbvPlanner<stateVec>::camVertical_ / 2.0) / 180.0
      + M_PI / 2.0;
  double camBottom = M_PI * (pitch + nbvInspection::nbvPlanner<stateVec>::camVertical_ / 2.0)
      / 180.0 - M_PI / 2.0;
...
nbvInspection::nbvPlanner<stateVec>::~nbvPlanner()
{
  ...
  for (typename std::vector<stateVec *>::iterator it = root_.begin(); it != root_.end(); it++) {
    if (*it) {
      delete *it;
    }
  }
  root_.clear();
  for (typename std::vector<stateVec *>::iterator it = g_stateOld_.begin(); it != g_stateOld_.end();
      it++) {
    if (*it) {
      delete *it;
    }
                                                      ...
                                                      int agentID)
{
  if (root_.size() <= agentID) {
    for (int it = root_.size(); it <= agentID; it++) {
      ROS_INFO("Adding new agent %i to NBV planner", it);
                                                          ...
                                                          int agentID)
{
  ...
  if (root_[agentID]->size() >= 9) {
    double dt = ros::Time::now().toSec() + dt_ + average_computation_duration_
        - (*root_[agentID])[8];
    ...
    double (nbvInspection::nbvPlanner<stateVec>::*informationGain)(stateVec), int agentID)
{
  ...
  while (nbvInspection::Node<stateVec>::bestInformationGain_
    ...
    if (volumetric_mapping::OctomapManager::CellStatus::kFree
        == this->manager_->getLineStatusBoundingBox(
            origin, direction + origin + direction.normalized() * tOvershoot, boundingBox_)) {
      ...
      tf::Quaternion quat;
      quat.setEuler(0.0, 0.0, newNode->state_[3]);
      p.pose.orientation.x = quat.x();
      p.pose.orientation.y = quat.y();
      p.pose.orientation.z = quat.z();
      p.pose.orientation.w = quat.w();
  ...
  if (curr->parent_ != NULL) {
    ...
    d = sqrt(d);
    double disc = nbvInspection::nbvPlanner<stateVec>::dt_
...
bool nbvInspection::nbvPlanner<stateVec>::setParams()
{
  ...
  if (!ros::param::get(ns + "/system/dv_max", dv_max_)) {
    ROS_WARN(
        "No maximal system acceleration specified (node: only used for euler integration tree extension). Looking for %s",
        (ns + "/system/v_max").c_str());
  }
  if (!ros::param::get(ns + "/system/ddyaw_max", ddyaw_max_)) {
    ROS_WARN(
        "No maximal yaw acceleration specified (node: only used for euler integration tree extension). Looking for %s",
  ...
  if (!ros::param::get(ns + "/nbvp/sampleHolonomic/extension_range", extensionRange_)) {
    ROS_WARN(
        "No value for maximal extension range specified (note: only needed for holonomic extension of tree). Looking for %s",
        (ns + "/nbvp/sampleHolonomic/extension_range").c_str());
  }
  if (!ros::param::get(ns + "/nbvp/RRT/initial_iterations", initIterations_)) {
    ROS_WARN(
        "No number of initial RRT iterations specified (note: only needed when RRT tree method is used). Looking for %s",
  ...
  if (!ros::param::get(ns + "/threshold_min", probability_mean_clamp_)
    ...
    ROS_WARN("No min clamp threshold value specified. Looking for %s",
             (ns + "/threshold_min").c_str());
    ret = false;
  }
  if (!ros::param::get(ns + "/threshold_max", probability_deviation_clamp_)
      && nbvInspection::nbvPlanner<stateVec>::igProbabilistic_ > 0.0) {
    ROS_WARN("No max clamp threshold value specified. Looking for %s",
             (ns + "/threshold_max").c_str());
    ret = false;
  }
...
template<typename stateVec>
Eigen::Vector3d nbvInspection::nbvPlanner<stateVec>::boundingBox_ = Eigen::Vector3d(0.5, 0.5, 0.3);
template<typename stateVec>
